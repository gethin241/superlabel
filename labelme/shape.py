...
class Shape(object):
    ...
    # TODO def polygon_to_mask(self, image_shape, x_offset=0, y_offset=0):
    def polygon_to_mask(self, image_shape, x_offset=0, y_offset=0):
        """
        将多边形转换为 NumPy 掩码。
        
        参数：
        - image_shape: 一个元组，表示掩码的形状 (height, width)。
        - polygon: 一个列表，其中包含表示多边形顶点的 (x, y) 坐标对。

        返回：
        - mask: 一个 NumPy 数组，表示多边形掩码。
        """
        polygon=[(point.x()+x_offset, point.y()+y_offset) for point in self.points]
        # 创建一个全零的掩码
        mask = np.zeros(image_shape, dtype=np.uint8)

        # 将多边形顶点转换为适用于 cv2.fillPoly 的格式
        polygon_pts = np.array(polygon, dtype=np.int32)

        # 填充多边形区域
        cv2.fillPoly(mask, [polygon_pts], color=1)
